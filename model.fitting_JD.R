...
ggplot(annuals, aes(x=pm2, y=percap, color=block)) +
  geom_jitter(aes(shape=warmtrt), width=.5)+
  geom_smooth(method = 'lm',formula = y ~ x + I(x^2), se=F)

ggplot(annuals, aes(x=seeded_a, y=percap, color=block)) +
  geom_jitter(aes(shape=warmtrt), width=.5)+
  geom_smooth(method = 'lm',formula = y ~ x + I(x^2), se=F)

ggplot(annuals, aes(x=pm2, y=seeded_a, color=block)) +
  geom_jitter(aes(shape=warmtrt), height=50)
  # geom_smooth(method = 'lm',formula = y ~ x + I(x^2), se=T)+
...
### BRM fit ----
#JD: trying percapita version & SCALED ----
#annual.simple <- brm(bf(plotseeds ~ (lambdaA*seeded_a) / (1+alphaAA*seeded_a + alphaAP*pm2), 
annual.simple <- brm(bf(percap ~ lambdaA*500 / (1 + alphaAA*seeded_a + alphaAP*pm2), 
                          lambdaA ~ warmtrt + (1|block), 
                          alphaAA ~  warmtrt + (1|block), 
...
#just using the basic data, ambient only, no blocks
annual.ambient <- brm(bf(percap ~ lambdaA / (1 + alphaAA*seeded_a + alphaAP*pm2), 
                        lambdaA ~ (1||block),
                        alphaAA ~  (1||block), 
                        alphaAP ~  (1||block), 
                        nl=TRUE),
                     data = filter(annuals, warmtrt=="amb"),
                     prior = c(prior(normal(500, 100), lb=0, nlpar = "lambdaA"), 
                               prior(normal(0, .1), nlpar = "alphaAA"),
                               prior(normal(0, .1), nlpar = "alphaAP")),
                     inits = "0",  
                     cores=4, 
